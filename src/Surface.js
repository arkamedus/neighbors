...
function Surface() {
    ...
    this._width = 500;
    this._height = 500;
    this._draw_textures = true;
    this._draw_stroke = false;
    this._draw_lighting = false;
    this._draw_outline = false;
...
Surface.prototype.drawFace = function (camera, tri, parent, texture, lod) {
...
/*
        ...
        if (subs < 2) {
            ...
            if (tri.center.dist(camera.from) > 2 + (2 * subs)) {


                //[t[0],t[1],t[2],
                //(t[0] + t[3]) / 2, (t[1] + t[4]) / 2, (t[2] + t[5]) / 2,
                //    (t[0] + t[6]) / 2, (t[1] + t[7]) / 2, (t[2] + t[8]) / 2]
                //[uv[0],uv[1], (uv[0] + uv[2]) / 2, (uv[1] + uv[3]) / 2, (uv[0] + uv[4]) / 2, (uv[1] + uv[5]) / 2 ]
                
                var divtri = (new Face3()).set(tri.pos1.x, tri.pos1.y, tri.pos1.z, tri.uv1.x, tri.uv1.y, (tri.pos1.x+tri.pos2.x)/2, (tri.pos1.y+tri.pos2.y)/2, (tri.pos1.z+tri.pos2.z)/2,(tri.uv1.x+tri.uv2.x)/2, (tri.uv1.y+tri.uv2.y)/2, (tri.pos1.x+tri.pos3.x)/2, (tri.pos1.y+tri.pos3.y)/2, (tri.pos1.z+tri.pos3.z)/2,(tri.uv1.x+tri.uv3.x)/2, (tri.uv1.y+tri.uv3.y)/2, tri.color1.r, tri.color1.g, tri.color1.b, tri.color2.r, tri.color2.g, tri.color2.b, tri.color3.r, tri.color3.g, tri.color3.b);
                this.drawFace(camera, divtri, parent, texture, subs + 1);

                //[t[3],t[4],t[5], (t[3] + t[6]) / 2, (t[4] + t[7]) / 2, (t[5] + t[8]) / 2, (t[0] + t[3]) / 2, (t[1] + t[4]) / 2, (t[2] + t[5]) / 2], [uv[2],uv[3], (uv[2] + uv[4]) / 2, (uv[3] + uv[5]) / 2, (uv[0] + uv[2]) / 2, (uv[1] + uv[3]) / 2 ]
               // this.drawFace(surface, divtri, parent, texture, subs + 1);

                //[t[6],t[7],t[8], (t[0] + t[6]) / 2, (t[1] + t[7]) / 2, (t[2] + t[8]) / 2, (t[3] + t[6]) / 2, (t[4] + t[7]) / 2, (t[5] + t[8]) / 2], [uv[4],uv[5], (uv[0] + uv[4]) / 2, (uv[1] + uv[5]) / 2, (uv[2] + uv[4]) / 2, (uv[3] + uv[5]) / 2 ]
                //this.drawFace(surface, divtri, parent, texture, subs + 1);

                //[(t[0] + t[6]) / 2, (t[1] + t[7]) / 2, (t[2] + t[8]) / 2, (t[3] + t[6]) / 2, (t[4] + t[7]) / 2, (t[5] + t[8]) / 2, (t[0] + t[3]) / 2, (t[1] + t[4]) / 2, (t[2] + t[5]) / 2], [(uv[0] + uv[4]) / 2, (uv[1] + uv[5]) / 2, (uv[2] + uv[4]) / 2, (uv[3] + uv[5]) / 2, (uv[0] + uv[2]) / 2, (uv[1] + uv[3]) / 2 ]
               // this.drawFace(surface, divtri, parent, texture, subs + 1);
...
*/
            ...
            this._tmp.drawntris++;
            camera.projection.toScreen(this, this._tmp.v1.copy(tri.pos1).mul(parent.scale).flipX().rotZ(-parent.rotation.z).rotX(parent.rotation.x).rotY(parent.rotation.y).add(parent.position), camera.from, this._tmp.p1);
            camera.projection.toScreen(this, this._tmp.v2.copy(tri.pos2).mul(parent.scale).flipX().rotZ(-parent.rotation.z).rotX(parent.rotation.x).rotY(parent.rotation.y).add(parent.position), camera.from, this._tmp.p2);
            camera.projection.toScreen(this, this._tmp.v3.copy(tri.pos3).mul(parent.scale).flipX().rotZ(-parent.rotation.z).rotX(parent.rotation.x).rotY(parent.rotation.y).add(parent.position), camera.from, this._tmp.p3);
...
if (!this._draw_only_outline) {
                ...
                if (this._draw_textures && texture) {
                    ...
                    //if (tri._dotvec.copy(tri.normal).rotZ(parent.rotation.z).dot(tri._cullvec.copy(camera.from).pointTo(camera.to))<=0.1){
                    this.context.save();
                    this.context.clip();
